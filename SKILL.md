---
name: markdown-unicode-pure
description: 严禁所有 Unicode 特殊字符（上下标、数学符号、希腊字母等），统一使用 ASCII + Markdown/LaTeX 语法。
---

# Unicode → ASCII 转换规范

## 自动化预处理（必做第一步）

批量文件先跑脚本做确定性替换，只对歧义清单做语义判断：

```bash
python3 unicode_purify.py <input> -o <output> --amb-out amb.json
```

- 脚本处理：数学运算符、希腊字母、标点、单位上标（`m²`→`m^2^`）、化学下标（`H₂O`→`H~2~O`）
- 脚本无法判断（标注 `[AMBIGUOUS]` 或写入 amb.json）：数学 vs 单位的上下标、`×` 尺寸 vs 乘法、`·` 分隔 vs 乘法
- 对 amb.json 每个条目，按下方决策树判断后替换
- 无脚本环境（或单段文本）：直接按下方规则转换

## 执行入口（决策树）

遇到特殊字符时，按优先级判断：

1. **Markdown 能简单表达？** → 用 Markdown（上下标一律 `^ ^` / `~ ~`，如 `m^2^`、`x^2^`、`H~2~O`、`E=mc^2^`；尺寸 `×` → 纯 ASCII `x`）
2. **Markdown 表达不了（数学运算符、希腊数学变量、复杂公式）？** → 完整 LaTeX `$...$`（如 `$\pm 2\%$`、`$\alpha$`、`$\sum x^2$`，LaTeX 内部用 `^`/`_`）
3. **序数/编号/年份？** → 保持纯文本（如 `1st`、`Figure 1`、`2024`）

优先级总原则：**Markdown > LaTeX > 绝不用 Unicode（任何形式）**。

## 转换规则表

### 数学运算符（→ LaTeX）

| 字符 | 替换 | 字符 | 替换 | 字符 | 替换 |
|------|------|------|------|------|------|
| `×` | `$\times$` | `≤` | `$\le$` | `∑` | `$\sum$` |
| `÷` | `$\div$` | `≥` | `$\ge$` | `∏` | `$\prod$` |
| `±` | `$\pm$` | `≠` | `$\neq$` | `∫` | `$\int$` |
| `∓` | `$\mp$` | `≈` | `$\approx$` | `∂` | `$\partial$` |
| `·` | `$\cdot$` | `≡` | `$\equiv$` | `∇` | `$\nabla$` |
| `−` | `$-$` | `∞` | `$\infty$` | `√` | `$\sqrt{}$` |
| `∈` | `$\in$` | `∉` | `$\notin$` | `∀` | `$\forall$` |
| `⊂` | `$\subset$` | `⊃` | `$\supset$` | `∃` | `$\exists$` |
| `∪` | `$\cup$` | `∩` | `$\cap$` | `∅` | `$\emptyset$` |

> 数学变量中的希腊字母：`$\alpha$` `$\beta$` `$\gamma$` `$\delta$` `$\epsilon$` `$\eta$` `$\theta$` `$\lambda$` `$\mu$` `$\nu$` `$\pi$` `$\sigma$` `$\phi$` `$\omega$`
> 非数学语境中的希腊字母：写英文名（`alpha`、`beta`、`eta`）

### Unicode 上下标（→ Markdown 优先）

| 场景 | 规则 | 示例 |
|------|------|------|
| 上标（任何语境） | `^数字^` | `²` → `m^2^`，`⁻¹` → `s^-1^`，`x²` → `x^2^` |
| 下标（任何语境） | `~数字~` | `₂` → `H~2~O`，`ᵢ` → `x~i~` |
| 仅当整个表达式是数学公式时 | 才用 `$...$`，内部 `^`/`_` | `$\sum x^2$`、`$E=mc^2$` |

### Unicode 标点（→ ASCII）

| 字符 | 替换 | 字符 | 替换 |
|------|------|------|------|
| `–` | `-` | `'` `'` | `'` |
| `—` | `--` | `"` `"` | `"` |
| `…` | `...` | | |

### 边界情况

| 场景 | 规则 |
|------|------|
| `×` 在尺寸描述中（如 `2.0 m × 1.4 m`） | 转纯 ASCII `x`（`2.0 m x 1.4 m`），不进 LaTeX |
| `×` 在数学公式中 | 用 `$\times$`（如 `$A = l \times w$`） |
| `·` 在数学中 → `$\cdot$`；在文本分隔中 → `.` | 看上下文 |
| `±2%` 等符号+数值 | 整体进 LaTeX `$\pm 2\%$`（`%` 须转义 `\%`，勿拆散） |
| `10^6^ kg` 等数量级+单位 | 用 `^ ^`，不进 LaTeX |

## 失败模式

| 触发条件 | 一线修复 | 兜底 |
|----------|---------|------|
| 未列出的 Unicode 字符 | 按决策树判断归属 | 标注 `[UNRESOLVED: U+XXXX]` |
| 字符歧义（如 `·`） | 检查上下文 | 默认转 LaTeX `$\cdot$` |
| LaTeX 语法不确定 | 按表格查表转换 | 标注 `[CHECK: ???]` |
| 输入无 Unicode 问题 | 原文输出 | — |
| 脚本已跑但歧义未决（amb.json 条目） | 按决策树逐条判断 | 无法判断则保留并标注 `[UNRESOLVED: U+XXXX]` |

## 常见错误

| 原句（❌） | 修正（✅） |
|-----------|-----------|
| 面积 10 m²，化学式 H₂O | 面积 10 m^2^，化学式 H~2~O |
| E=mc²，第 2nd 章讨论 ¹⁴C | $E=mc^2$，第 2nd 章讨论 ^14^C |
| 加速度 m/s² | 加速度 m/s^-2^ |
| 效率 η 95%，误差 ±2% | 效率 $\eta$ 95%，误差 $\pm 2\%$ |
| 浓度 0.1−0.5 mol/L，温度 20–25°C | 浓度 0.1-0.5 mol/L，温度 20-25°C |
| 面积 2.0 m × 1.4 m，体积 2.8 m³ | 面积 2.0 m × 1.4 m，体积 2.8 m^3^ |

## 反例清单（不要做什么）

| ❌ 不要 | ✅ 要 |
|--------|------|
| 不要用 Unicode 上下标 `m²` `H₂O` | 用 Markdown `m^2^` `H~2~O` |
| 不要用 Unicode 希腊字母 `η` `α` `β` | 数学中用 `$\eta$`；文本用 `eta` `alpha` |
| 不要用 Unicode 数学符号 `±` `≤` `≥` | 用 LaTeX `$\pm$` `$\le$` `$\ge$` |
| 不要用 Unicode 标点 `–` `—` `…` | 用 ASCII `-` `--` `...` |
| 不要把单位写进 LaTeX `$10 m^2$` | 用 Markdown `10 m^2^` |
| 不要把化学式写进 LaTeX `$H_2O$` | 用 Markdown `H~2~O` |
| 不要把序数写成上标 `1^st^` | 保持纯文本 `1st` |
