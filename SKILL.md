...
| 仅当整个表达式是数学公式且含运算符时 | 才用 `$...$`，内部 `^`/`_`；`∑`/`∫`/`∏` 后**整体吸收**后续表达式进同一块 | `$\sum x^2$`、`$\int 0^1 x^2 dx$`（`E=mc²` 无运算符 → `E=mc^2^`） |
...
| `∑`/`∫`/`∏` 后接表达式 | **整体吸收**进一个 LaTeX 块（`∑x²`→`$\sum x^2$`、`∫0¹ x² dx`→`$\int 0^1 x^2 dx$`）；遇中文（`和`、`到`、`，`）立即中断，中文绝不放进 LaTeX |
